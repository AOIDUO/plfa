...
...                      | no A≠    | xx = no ( λ { refl → A≠ refl } )
...                      | yes refl | no B≠ = no ( λ { refl → B≠ refl } )
...                      | yes refl | yes refl = yes refl
-- end
```
...
uniq-↑ (⊢↓ ⊢M) (⊢↓ ⊢M′)       =  refl
-- begin
uniq-↑ {M = `proj₁ M} (⊢`proj₁ x) (⊢`proj₁ x₁) = `×≡l (uniq-↑ x x₁) 
uniq-↑ {M = `proj₂ M} (⊢`proj₂ x) (⊢`proj₂ x₁) = `×≡l (uniq-↑ x x₁) 
-- end
```
...
... | no ¬⊢M = no (λ { ⟨ fst , ⊢`proj₁ ⊢M ⟩ →  ¬⊢M ⟨ _ , ⊢M ⟩ }) 
... | yes ⟨ `ℕ , ⊢n ⟩ = no (λ { ⟨ fst , ⊢`proj₁ ⊢M ⟩ → `×≢ℕ (uniq-↑ ⊢M ⊢n) } )
... | yes ⟨ fst ⇒ fst₁ , ⊢M⇒ ⟩ = no λ { ⟨ fst₂ , ⊢`proj₁ ⊢M× ⟩ → `×≢⇒ (uniq-↑ ⊢M× ⊢M⇒ )  }
... | yes ⟨ fst `× fst₁ , ⊢M ⟩ = yes ⟨ fst , (⊢`proj₁ ⊢M ) ⟩
synthesize Γ (`proj₂ M) with synthesize Γ M
... | no ¬⊢M = no (λ { ⟨ fst , ⊢`proj₂ ⊢M ⟩ →  ¬⊢M ⟨ _ , ⊢M ⟩ }) 
... | yes ⟨ `ℕ , ⊢n ⟩ = no (λ { ⟨ fst , ⊢`proj₂ ⊢M ⟩ → `×≢ℕ (uniq-↑ ⊢M ⊢n) } )
... | yes ⟨ fst ⇒ fst₁ , ⊢M⇒ ⟩ = no λ { ⟨ fst₂ , ⊢`proj₂ ⊢M× ⟩ → `×≢⇒ (uniq-↑ ⊢M× ⊢M⇒ )  }
...
...   | yes A≡B             =  yes (⊢↑ ⊢M A≡B)
-- begin
inherit Γ `⟨ M , M₁ ⟩ `ℕ = no (λ ())
inherit Γ `⟨ M , M₁ ⟩ (A ⇒ A₁) = no (λ ())
inherit Γ (ƛ x ⇒ M) (A `× A₁) = no (λ ())
inherit Γ `zero (A `× A₁) = no (λ ())
inherit Γ (`suc M) (A `× A₁) = no (λ ())
