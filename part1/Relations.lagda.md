...
```agda
-- the relation "is big O notation of"
```
...
```agda
-- When `z≤n` is the first argument and `s≤s` is the second, we have: 
-- `≤-antisym {suc m} {suc n} z≤n (s≤s m n)`
-- This implies that `{suc m} ≡ zero`, which is impossible, since zero cannot be 
-- expressed as a successor of some nature number. 
```
...
data One : Bin → Set where
  one : One (⟨⟩ I)
  one_suc : ∀ {b} → One b → One (inc b)

data Can : Bin → Set where
  zero : Can (⟨⟩ O)
  mkcan : ∀ {b : Bin} → One b → Can b
  
can-inc : {b : Bin} → Can b → Can (inc b)
can-inc zero = mkcan one
can-inc (mkcan oneb) = mkcan (one_suc oneb)

can-to : (m : ℕ) → Can (to m)
can-to zero = zero
can-to (suc m) = can-inc (can-to m)

-- can-from : {b : Bin} → Can b → (from b)
-- can-from : {b : Bin} → Can b → ℕ
-- can-from zero = zero
-- can-from (mkcan one) = 1
-- can-from (mkcan (one_suc oneb)) = suc ((can-from (mkcan oneb)))


-- can-from (mkcan (one_suc x)) = suc (can-from (mkcan x))
-- from (b O) = (from b) * 2
-- from (b I) = (from b) * 2 + 1
-- from ⟨⟩ = 0
