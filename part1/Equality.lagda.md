...
even-comm′ m n even_m+n with     m + n  | +-comm m n
even-comm′ m n even_m+n     | .(n + m)  | refl               = even_m+n


Reven-comm′ : ∀ (n m : ℕ) → even (n + m) → even (m + n)
Reven-comm′ n m even_n+m with n + m | +-comm n m
...                         | .(m + n) | refl = even_n+m
...
even+-assoc : ∀ (m n p : ℕ) → even ( (m + n)  + p  ) → even ( m + (n + p)) 
even+-assoc m n p even_m+n+p rewrite +-assoc m n p  = even_m+n+p

even+-assoc` : ∀ (m n p : ℕ) → even ( (m + n)  + p  ) → even ( m + (n + p)) 
even+-assoc` m n p even_m+n+p with (m + n)  + p  | +-assoc m n p 
...                          | .( m + (n + p)) | refl = even_m+n+p
```
